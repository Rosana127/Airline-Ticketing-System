...
forgetPassword2::forgetPassword2(QWidget *parent, bool isAdmin)
    : QWidget(parent), isAdmin(isAdmin), ui(new Ui::forgetPassword2)  // 初始化 isAdmin 成员变量
{
    ui->setupUi(this);
    generatedCode = "";
}
...
void forgetPassword2::on_getCheckButton_clicked()
{
    // 获取用户输入的用户名和手机号
    ...
    if(!isAdmin){
        ...
        query.prepare("SELECT COUNT(*) FROM managerinfo WHERE ManagerName = :username ");
        query.bindValue(":username", username);
    }
    if (!query.exec()) {
    ...
    if (count == 0) {
        // 用户名和手机号不匹配
        QMessageBox::warning(this, "验证失败", "未找到该用户，请先注册！");
        return ;
    }
...
void forgetPassword2::on_forgetButton_clicked()
{
    // 获取用户输入的验证码
    ...
    if (inputCheck != generatedCode) {
        ...
        if(!isAdmin){
            ...
            query.prepare("SELECT Password FROM managerinfo WHERE ManagerName = :username AND PhoneNumber = :phone");
            query.bindValue(":username", username);
            query.bindValue(":phone", phone);
        }
        if (!query.exec()) {
